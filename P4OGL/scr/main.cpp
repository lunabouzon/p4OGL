...
//VAO
unsigned int vao;
unsigned int planeVAO;	//variables que nos dan acceso al objeto cuadrado
...
unsigned int planeVertexVBO;  //variables que nos dan acceso al objeto cuadrado
...
void destroy()
{
	...
	//borramos los shaders post proceso
...
void renderFunc()
{
	...
	/*glEnable(GL_BLEND);
	//glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); //minus source alpha evita que se pinte el fondo por encima de la imagen de delante

	glBlendFunc(GL_CONSTANT_COLOR, GL_CONSTANT_ALPHA); //source y destino
